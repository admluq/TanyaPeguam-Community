        ...
        <div className="glass-card rounded-2xl p-8">
          {registered && (
            <div className="mb-5 flex items-start gap-3 p-3 rounded-lg bg-[rgba(52,211,153,0.12)] border border-[rgba(52,211,153,0.2)]">
              <span className="text-sm text-[#34d399]">✓ Pendaftaran berjaya! Sila log masuk di bawah.</span>
            </div>
          )}
          ...
          {/* Tabs */}
          <div className="flex gap-2 mb-6 bg-bg-3 p-1 rounded-xl">
            <button
              onClick={() => { setTab('google'); setError(null); }}
              className={`flex-1 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                tab === 'google'
                  ? 'bg-lia-dim text-lia-light'
          ...
          {tab === 'google' ? (
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-semibold py-3.5 px-6 rounded-xl hover:bg-gray-50 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-150 shadow-md"
            >
              <GoogleIcon />
            ...
            <form onSubmit={handleEmailSignIn} className="space-y-4">
              ...
              >
                {isLoading ? (
                  <><Loader2 size={15} className="animate-spin" /> Sedang log masuk...</>
                ) : (
                  'Log Masuk'
                )}
              </button>
